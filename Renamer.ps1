...
Get-ChildItem -Path $PSScriptRoot -File -Recurse -exclude *.ps1 | % {
    ...
    if ($contents -match "resident-vulnerabilities-api" -or $contents -match "resident_vulnerabilities_api") {
        $contents -replace 'resident-vulnerabilities-api', $apiName -replace 'resident_vulnerabilities_api', $apiName | Set-Content $_.PSPath
        Write-Host $("'{0}': contents changed." -f $fileName)
    }
    
    if ($fileName -match 'resident-vulnerabilities-api') {
        $newName = $_.Name -replace 'resident-vulnerabilities-api', $apiName
...
Where-Object { $_.Name -match 'resident-vulnerabilities-api' } | % {
    Write-Host $("Editing directory: '{0}'." -f $_.FullName)
    $newDirName = $_.Name -replace 'resident-vulnerabilities-api', $apiName
